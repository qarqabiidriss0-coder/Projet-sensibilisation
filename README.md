...
⚠️ **Avertissement Éthique & Légal :** Cet outil a été créé à des fins strictement pédagogiques et d'audit interne. Il ne doit en aucun cas être utilisé pour mener de véritables attaques malveillantes ou usurper des identités sans le consentement explicite des parties concernées.
...
1.  **Cloner le dépôt :**
```bash
git clone https://github.com/qarqabiidriss0-coder/Projet-sensibilisation.git
cd Projet-sensibilisation
```
...
Ce projet est optimisé pour un déploiement fluide sur Vercel. Les fichiers `vercel.json` et `next.config.ts` ont été configurés pour assurer une mise en production continue.
